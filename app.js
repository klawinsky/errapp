/* app.js
   eRegioJet — kompletny plik frontu (gotowy do wklejenia)
   - wymaga: window.supabase zainicjalizowanego w app.html (anon key)
   - używa lokalnej zmiennej `sb` zamiast redeklaracji globalnego `supabase`
   - zawiera: auth check, inicjalizację DOM po load, CRUD raportów i powiązanych tabel,
     modale, render, druk, oraz funkcje handover/takeover
*/
...
async function addConsist(reportId, type, mark, fromStation, toStation) {
  ...
  const { data, error } = await sb.from('consist').insert([{
    report_id: reportId, type, mark, from_station: fromStation || '', to_station: toStation || ''
  }]).select();
...
async function addCrew(reportId, name, role, fromStation, toStation) {
  ...
  const { data, error } = await sb.from('crew').insert([{
    report_id: reportId, name, role, from_station: fromStation || '', to_station: toStation || ''
  }]).select();
...
function bindUiActions() {
  ...
  if (qs("#handover-btn")) qs("#handover-btn").addEventListener("click", async () => {
    // właściciel przekazuje obsługę (ustawia status = 'handed_over')
    if (!currentReportId) { alert('Brak aktywnego raportu'); return; }
    await handoverReport(currentReportId);
  });

  // New report button (if not bound elsewhere)
  ...
  // Podłącz przyciski takeover/preview w listach (delegacja)
  document.addEventListener("click", async (e) => {
    const btn = e.target.closest("button");
    if (!btn) return;
    const action = btn.getAttribute("data-action");
    ...
    const id = btn.getAttribute("data-id");
    if (action === "takeover") {
      // inny użytkownik przejmuje raport
...
/* ---------- Handover / Takeover functions ---------- */

/**
 * Właściciel ustawia status = 'handed_over' (udostępnia do przejęcia).
 * Polityka RLS powinna pozwalać właścicielowi na update statusu.
 */
async function handoverReport(reportId) {
  ...
  if (error) {
    ...
    alert('Błąd przy przekazywaniu obsługi: ' + (error.message || JSON.stringify(error)));
    return;
  }
  const report = await getReportById(reportId);
  loadReportIntoForm(report, true); // read-only after handing over
  refreshLists();
}

/**
 * Inny użytkownik przejmuje raport:
 * - musi być aktualnie status = 'handed_over' (polityka USING)
 * - update ustawia created_by = auth.uid() i status = 'in_progress' (polityka WITH CHECK)
 */
async function takeOverReport(reportId) {
  ...
  if (error) {
    ...
    alert('Błąd przy przejmowaniu raportu: ' + (error.message || JSON.stringify(error)));
    return;
  }
...
async function refreshLists() {
  ...
  // check (today/yesterday) - wszyscy uwierzytelnieni widzą raporty (zgodnie z polityką SELECT)
...
function openPrintWindow(report) {
  ...
  const html = `
    ...
    <div class="section"><h2>Dane ogólne</h2><table><tr><th>Numer pociągu</th><td>${escapeHtml(report.general.trainNumber || '')}</td></tr><tr><th>Dzień kursowania</th><td>${escapeHtml(report.general.date || '')}</td></tr><tr><th>Stacja od</th><td>${escapeHtml(report.general.from || '')}</td></tr><tr><th>Stacja do</th><td>${escapeHtml(report.general.to || '')}</td></tr></table></div>
    <div class="section"><h2>Skład pociągu</h2><table><thead><tr><th>Typ</th><th>Oznaczenie</th><th>Od</th><th>Do</th></tr></thead><tbody>${(report.consist||[]).map(l=>`<tr><td>${escapeHtml(l.type==='loco'?'Lok':'Wagon')}</td><td>${escapeHtml(l.mark)}</td><td>${escapeHtml(l.from_station)}</td><td>${escapeHtml(l.to_station)}</td></tr>`).join('')}</tbody></table></div>
    <div class="section"><h2>Drużyna pociągowa</h2><table><thead><tr><th>Imię i nazwisko</th><th>Funkcja</th><th>Od</th><th>Do</th></tr></thead><tbody>${(report.crew||[]).map(c=>`<tr><td>${escapeHtml(c.name)}</td><td>${escapeHtml(c.role)}</td><td>${escapeHtml(c.from_station)}</td><td>${escapeHtml(c.to_station)}</td></tr>`).join('')}</tbody></table></div>
    <div class="section"><h2>Dane o jeździe pociągu</h2><table><thead><tr><th>Stacja</th><th>Plan przyj.</th><th>Rzecz. przyj.</th><th>Odch. (min)</th><th>Plan odj.</th><th>Rzecz. odj.</th><th>Odch. (min)</th><th>Powód</th><th>Rozkazy</th></tr></thead><tbody>${(report.runs||[]).map(r=>{const arrDelay=calculateDelayMinutes(r.planned_arr,r.actual_arr);const depDelay=calculateDelayMinutes(r.planned_dep,r.actual_dep);return `<tr><td>${escapeHtml(r.station)}</td><td>${escapeHtml(r.planned_arr||"")}</td><td>${escapeHtml(r.actual_arr||"")}</td><td>${arrDelay===null?"":(arrDelay>0?("+"+arrDelay):arrDelay)}</td><td>${escapeHtml(r.planned_dep||"")}</td><td>${escapeHtml(r.actual_dep||"")}</td><td>${depDelay===null?"":(depDelay>0?("+"+depDelay):depDelay)}</td><td>${escapeHtml(r.delay_reason||"")}</td><td>${escapeHtml(r.orders||"")}</td></tr>`;}).join('')}</tbody></table></div>
    <div class="section"><h2>Dyspozycje</h2><table><thead><tr><th>Źródło</th><th>Treść</th></tr></thead><tbody>${(report.dispos||[]).map(d=>`<tr><td>${escapeHtml(d.source)}</td><td>${escapeHtml(d.text)}</td></tr>`).join('')}</tbody></table></div>
    <div class="section"><h2>Uwagi kierownika</h2><table><tbody>${(report.remarks||[]).map(r=>`<tr><td>${escapeHtml(r.text)}</td></tr>`).join('')}</tbody></table></div>
...
async function initApp() {
  ...
  // Initial load of reports and UI
  try {
